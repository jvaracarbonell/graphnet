...
class IceCubeGen2SUM(Detector):
    ...
    def feature_map(self) -> Dict[str, Callable]:
        ...
        feature_map = {
            ...
            "sum_5": self._sum_times, # features 6 and 5 are very related
            "sum_6": self._sum_times,
            "sum_7": self._sum_times,
            "sum_8": self._sum_times,
            "sum_9": self._sum_9,
    ...
    # Scale everything rougly either 0 - 1 (charges) or -1 - 1 (times)
    def _sum_charges(self, x: torch.tensor) -> torch.tensor:
        return torch.log10(x+1) / 3 
    
    def _sum_times(self, x: torch.tensor) -> torch.tensor:
        return ((x + 1e4) / 3e4)* 2 -1
